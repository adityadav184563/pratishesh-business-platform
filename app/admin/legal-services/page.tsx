...
import {
  createLegalServiceAdmin,
  deleteLegalServiceAdmin,
  fetchLegalServicesAdmin,
  updateLegalServiceAdmin,
} from "@/lib/api-client"
...
export default function AdminLegalServicesPage() {
  ...
  const [items, setItems] = useState<LegalService[]>([])
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      setItems(await fetchLegalServicesAdmin())
    } finally {
      setLoading(false)
    }
  }, [])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true)
    try {
      if (editing) await updateLegalServiceAdmin(editing.id, form)
      else await createLegalServiceAdmin(form)
      setModalOpen(false)
      await load()
    } catch (err) {
  ...
  return (
    <AdminLayout
      ...
      action={
        <button type="button" onClick={() => { setEditing(null); setForm({ ...empty, order: items.length + 1 }); setModalOpen(true) }} className="inline-flex items-center gap-2 rounded-lg bg-orange-500 px-4 py-2 text-sm font-semibold text-white hover:bg-orange-600">
        ...
        <DataTable
          ...
          actions={(row) => (
            <div className="flex gap-2">
              <button type="button" onClick={() => { setEditing(row); setForm({ title: row.title, description: row.description, icon: row.icon, category: row.category, status: row.status, order: row.order }); setModalOpen(true) }} className="rounded p-1 hover:text-orange-600"><Pencil className="h-4 w-4" /></button>
              <button type="button" onClick={async () => { if (confirm("Delete?")) { await deleteLegalServiceAdmin(row.id); load() } }} className="rounded p-1 hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
            </div>
          )}
        />
      )}
      <FormModal open={modalOpen} title={editing ? "Edit Legal Service" : "Add Legal Service"} onClose={() => setModalOpen(false)} onSubmit={handleSubmit} submitLabel={saving ? "Saving..." : "Save"}>
