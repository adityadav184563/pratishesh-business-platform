...
import { fetchSettingsAdmin, updateSettingsAdmin } from "@/lib/api-client"
...
export default function AdminSettingsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      setForm(await fetchSettingsAdmin())
    } finally {
      setLoading(false)
    }
  }, [])
      ...
      {loading ? (
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl space-y-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="flex items-center gap-4 pt-2">
            <button type="submit" disabled={saving} className="inline-flex items-center gap-2 rounded-lg bg-orange-500 px-5 py-2.5 text-sm font-semibold text-white hover:bg-orange-600 disabled:opacity-60">
