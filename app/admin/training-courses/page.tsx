...
import { getTrainingCourses, saveTrainingCourses, uid } from "@/lib/adminData"
...
export default function AdminTrainingCoursesPage() {
  ...
  const [items, setItems] = useState<TrainingCourse[]>(() => getTrainingCourses())
  const [modalOpen, setModalOpen] = useState(false)
  const [editing, setEditing] = useState<TrainingCourse | null>(null)
  const [form, setForm] = useState(empty)

  useEffect(() => setItems(getTrainingCourses()), [tick])
  ...
  return (
    ...
    >
      ...
      />
      <FormModal open={modalOpen} title={editing ? "Edit Course" : "Add Course"} onClose={() => setModalOpen(false)} onSubmit={handleSubmit}>
        ...
        <FormField
          ...
          as="select"
          value={form.placementSupport ? "yes" : "no"}
          onChange={(v) => setForm({ ...form, placementSupport: v === "yes" })}
