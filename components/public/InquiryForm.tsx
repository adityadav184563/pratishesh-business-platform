...
import { addInquiry } from "@/lib/adminData"
...
export default function InquiryForm({ division, className = "" }: InquiryFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    addInquiry({ name, email, phone, message, division })
    setSubmitted(true)
    setName("")
    setEmail("")
    setPhone("")
  ...
  return (
    <form className={`space-y-5 ${className}`} onSubmit={handleSubmit}>
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <div>
          <label htmlFor="inq-email" className="mb-1.5 block text-sm font-medium text-foreground">
            Email
          </label>
          <input
            id="inq-email"
      ...
      <div>
        <label htmlFor="inq-phone" className="mb-1.5 block text-sm font-medium text-foreground">
          Phone Number
        </label>
        <input
          id="inq-phone"
      ...
      {submitted && (
        <p className="text-sm text-emerald-400">Thank you! Your message has been sent. We will contact you soon.</p>
      )}
      <button
        type="submit"
